...
{
    /* desiredAccess=0 + share R+W = "permissive query": coexists with apps
     * holding the device for exclusive read (Steam Input, gamepad-tester
     * browser tabs, GameSir software, Windows HID class driver). With
     * GENERIC_READ here we'd silently fail to enumerate any controller
     * already open elsewhere. HidD_GetAttributes works fine on a
     * zero-access handle since it queries device-class info, not data. */
    HANDLE dev_handle = CreateFile(path, 0, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, 0);
